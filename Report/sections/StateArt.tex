...
\end{equation}
where $\rho$ is the density, $\vec{v}$ the velocity, $\Gamma$ the diffusion coefficient, $S_{\phi}$ the source term, and $\phi$ the general variable that is going to be studied. In table \ref{convdifeq} there are the values that these variables have to take in order to obtain the conservation equations previously mentioned.
...
\subsection{Mesh}
There are different ways to discretize the domain for the FVM. These methods are listed below \cite{Ferziger2002}:
\begin{itemize}
...
Time is a one-way variable, which means that the unknowns only depend on the values in the previous instant of time, and do not depend on the values in the next instant of time. Knowing this property, to obtain the results of an unsteady problem, the method used is to discretize the time domain in time steps and calculate the values in each of them. When the unknowns of one time step are obtained, the calculation moves on to the next time step.
