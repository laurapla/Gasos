...
double time_step (float dtd, float* x, float* y, float* xvc, float* yvc, staggx u, staggy v);
...
int main()
{
	...
	while(resta>delta)
	{
		...
		Gauss_Seidel (ap, aw, ae, as, an, bp, fr, delta, N, M, p);
		...
		// STEP 4 !!! : TIME STEP
		cout<<"Time Step	";
		dt = time_step (dtd, x, y, xvc, yvc, u, v);
...
//		cout<<Sh[i]<<endl;
//	}
//	output_matrix(N, M, aw);
	
	ofstream results;
    results.open("Results.dat");
    for(int i = 0; i<=N; i++)
    {
    	results<<x[i]<<"	"<<u[M/2][i]<<endl;
	}
    results.close();
...
double time_step (float dtd, float* x, float* y, float* xvc, float* yvc, staggx u, staggy v)
{
	...
	for(int i = 0; i<N+1; i++)
	{
		for(int j = 0; j<M; j++)
		{
			if(i==0)
			{
				dtc = min(dtc, 0.35*fabs(x[i]-xvc[i])/fabs(u[j][i]));
			}
			else if(i==N)
			{
				dtc = min(dtc, 0.35*fabs(xvc[i]-x[i-1])/fabs(u[j][i]));
			}
			else
			{
				dtc = min(dtc, 0.35*fabs(x[i]-x[i-1])/fabs(u[j][i]));
			}
	...
	for(int i = 0; i<N; i++)
	{
		for(int j = 0; j<M+1; j++)
		{
			if(j==0)
			{
				dtc = min(dtc, 0.35*fabs(y[j]-yvc[j])/fabs(v[j][i]));
			}
			else if(j==M)
			{
				dtc = min(dtc, 0.35*fabs(yvc[j]-y[j-1])/fabs(v[j][i]));
			}
			else
			{
				dtc = min(dtc, 0.35*fabs(y[j]-y[j-1])/fabs(v[j][i]));
			}
		}
	}
	dt = min(dtc, dtd);
